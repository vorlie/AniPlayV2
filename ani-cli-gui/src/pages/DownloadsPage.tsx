...
function statusLabel(job: DownloadJob): string {
  ...
  if (job.status === 'resolving') return 'Finding a fresh stream…'
  if (job.status === 'downloading') return job.progress.percent == null ? 'Downloading…' : `${Math.round(job.progress.percent)}%`
  ...
  return (
    <div className="flex-1 flex flex-col gap-4">
      <section className="m3-card p-5 md:p-7 flex items-start justify-between gap-4">
        <div>
          <p className="section-label"><Download size={14} /> Downloads</p>
          <h2 className="mt-2 text-3xl font-black tracking-tight">Your offline queue</h2>
          <p className="mt-1 text-sm text-m3-on-surface-variant">{activeCount ? `${activeCount} active or queued` : 'No active downloads'} · {state?.settings.directory ?? 'Loading folder…'}</p>
        </div>
        <button type="button" onClick={() => window.aniPlay && invoke(window.aniPlay.downloads.clearFinished())} className="icon-button" title="Clear finished downloads">
          <Trash2 size={18} />
        </button>
      </section>
      ...
      {state && !state.ffmpegAvailable && (
        <div role="alert" className="rounded-2xl border border-red-400/20 bg-red-400/10 p-4 text-sm text-red-200 flex gap-3">
          ...
          <div><p className="font-bold">FFmpeg unavailable</p><p className="mt-1 opacity-80">{state.ffmpegError}</p></div>
        </div>
      )}
      ...
      <section className="m3-card p-4 md:p-6 flex-1 min-h-[320px]">
        {jobs.length === 0 ? (
          <div className="min-h-[280px] flex flex-col items-center justify-center text-center">
            ...
            <p className="mt-4 font-bold">Nothing downloaded yet</p>
            <p className="mt-1 max-w-md text-sm text-m3-on-surface-variant">Open an episode, select the server and quality you want, then use Download in the player.</p>
          </div>
        ) : (
                ...
                <article key={job.id} className="rounded-2xl border border-m3-outline/15 bg-m3-surface/35 p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex flex-wrap items-start justify-between gap-2">
                        <div className="min-w-0">
                          <h3 className="truncate font-bold">{job.request.animeName} · Episode {job.request.episode}</h3>
                          <p className="mt-0.5 text-xs text-m3-on-surface-variant">{job.request.provider} · {job.request.resolution} · {job.request.translationType.toUpperCase()}{job.status === 'queued' ? ` · Queue ${queuedIds.indexOf(job.id) + 1}` : ''}</p>
                        </div>
                        <span className="text-xs font-bold text-m3-primary">{statusLabel(job)}</span>
                      </div>
                    ...
                    <div className="flex shrink-0 gap-1">
                      {active && <button type="button" className="icon-button !size-9" title="Cancel" onClick={() => window.aniPlay && invoke(window.aniPlay.downloads.cancel(job.id))}><X size={16} /></button>}
                      {canRetry && <button type="button" className="icon-button !size-9" title="Retry" onClick={() => window.aniPlay && invoke(window.aniPlay.downloads.retry(job.id))}><RefreshCw size={16} /></button>}
                      {job.status === 'completed' && <button type="button" className="icon-button !size-9" title="Show in folder" onClick={() => window.aniPlay && invoke(window.aniPlay.downloads.reveal(job.id))}><FolderOpen size={16} /></button>}
                    </div>
                  </div>
                </article>
              )
