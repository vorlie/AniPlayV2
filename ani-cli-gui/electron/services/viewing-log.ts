...
export function validateWatchSegment(value: unknown): WatchSegmentInput {
  ...
  const catalogProvider = item.catalogProvider === 'desu' || item.catalogProvider === 'docchi' || item.catalogProvider === 'anidb' || item.catalogProvider === 'anikoto' || item.catalogProvider === 'anikoto2' ? item.catalogProvider : null
