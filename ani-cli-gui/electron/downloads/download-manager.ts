...
export class DownloadManager {
  ...
  private async run(job: DownloadJob): Promise<void> {
    ...
    try {
      ...
      try {
        await this.runFfmpeg(job, relay?.url ?? link.url, partialPath, link.subtitles ?? [], link.requestHeaders)
      } finally {
        await relay?.close()
      }
  ...
  private runFfmpeg(job: DownloadJob, url: string, partialPath: string, subtitles: { label: string; url: string }[], requestHeaders?: Record<string, string>): Promise<void> {
    const args = buildFfmpegArgs(url, partialPath, subtitles, requestHeaders)
